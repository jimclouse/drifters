...
into outfile '/tmp/drifters_pac_1200_end_2011.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\r\n'
from gdpPacAll as a 
join (      select g.id from (select distinct id from gdpPacAll) as g
            where exists ( select 1 from gdpPacAll s2 where s2.id = g.id and s2.obsDate = '2011-03-31') 
               and exists ( select 1 from gdpPacAll s3 where s3.id = g.id and s3.obsDate = '2007-12-17' and s3.latitude > 0 )
) as sub
on a.id = sub.id
